    ...
    <main className="mx-auto min-h-screen w-[min(1080px,calc(100%-32px))] py-7">
      ...
      <section className="mx-auto mb-10 mt-16 max-w-3xl text-center md:mt-24">
        <span className="inline-flex items-center gap-2 font-mono text-[11px] font-bold tracking-[.15em] text-[#6d45e8]">
          <Sparkles size={14} /> ENSAIO COM EVIDÊNCIA
        </span>
        <h1 className="mt-5 text-5xl font-semibold leading-[1.02] tracking-[-.055em] md:text-7xl">
          Treine com seu cliente<br />antes de falar com ele.
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-[#716d77] md:text-lg">
          Cole o histórico real. O Espelho aprende somente o que pode provar —
          e vira o cliente para você praticar ao vivo.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-[860px] rounded-3xl border border-[#e2dfdb] bg-white p-5 shadow-[0_22px_70px_rgba(32,26,43,.07)] md:p-7">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-semibold">Cole o histórico do cliente</h2>
          </div>
          <span className="hidden text-xs text-[#97929b] sm:block">Nada será enviado ao cliente</span>
        </div>
        <textarea
        ...
        <div className="mt-4 flex flex-col-reverse justify-between gap-3 sm:flex-row">
          ...
          <button
            ...
            onClick={onSubmit}
          >
            {loading ? "Criando espelho..." : "Criar espelho"}
      ...
      <div className="mx-auto mt-8 flex max-w-sm items-center justify-center gap-3 font-mono text-[9px] tracking-[.16em] text-[#aaa6ae]">
        <strong className="text-[#6d45e8]">COLAR</strong><i className="h-px w-12 bg-[#d8d4d9]" />
        <span>PRATICAR</span><i className="h-px w-12 bg-[#d8d4d9]" /><span>REFAZER</span>
      </div>
    </main>
  );
}
  ...
  const evidenceCard = (fact: CustomerTwin["facts"][number]) => (
    <article className="rounded-2xl border border-[#e7e3df] bg-white p-4" key={fact.id}>
      ...
      <span className="font-mono text-[10px] text-[#388761]">
        Histórico · mensagem {fact.evidence[0]?.sourceIndex}
      </span>
    </article>
  );
  ...
  return (
    <main className="mx-auto min-h-screen w-[min(1100px,calc(100%-32px))] py-7">
      ...
      <header className="my-9 grid items-center gap-5 md:grid-cols-[72px_1fr_auto]">
        ...
        <div>
          <span className="font-mono text-[11px] font-bold tracking-[.14em] text-[#6d45e8]">ESPELHO CRIADO</span>
          <h1 className="mt-1 text-4xl font-semibold tracking-[-.04em]">Conheça {twin.name}</h1>
          <p className="mt-1 text-[#76717b]">Tudo que este espelho sabe vem do histórico que você forneceu.</p>
        </div>
        <button className={buttonPrimary} type="button" onClick={onStart}>
          <Mic size={18} /> Ensaiar conversa
        </button>
      </header>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[1.2fr_1fr_.9fr]">
        <section className="rounded-3xl border border-[#e2dfdb] bg-white/70 p-5">
          <div className="mb-4 flex items-center gap-2 font-mono text-[10px] font-bold tracking-[.13em]">
            <i className="size-2 rounded-full bg-[#2f8f65]" /> SABEMOS <span className="ml-auto text-[#aaa6ae]">{known.length}</span>
          </div>
        ...
        <section className="rounded-3xl border border-[#e2dfdb] bg-white/70 p-5">
          <div className="flex items-center gap-2 font-mono text-[10px] font-bold tracking-[.13em]">
            <i className="size-2 rounded-full bg-[#bd7b1b]" /> PROVAVELMENTE <span className="ml-auto text-[#aaa6ae]">{likely.length}</span>
          </div>
          <p className="mb-4 mt-2 text-xs text-[#96919a]">Inferências apoiadas pelo histórico</p>
          <div className="grid gap-3">{likely.slice(0, 2).map(evidenceCard)}</div>
        </section>
        <section className="rounded-3xl border border-[#e2dfdb] bg-white/70 p-5">
          <div className="flex items-center gap-2 font-mono text-[10px] font-bold tracking-[.13em]">
            <i className="size-2 rounded-full bg-[#aaa6ae]" /> NÃO SABEMOS <span className="ml-auto text-[#aaa6ae]">{twin.unknowns.length}</span>
          </div>
          <p className="mb-4 mt-2 text-xs text-[#96919a]">{twin.name} não inventará respostas</p>
  ...
  return (
    <main className="flex min-h-screen flex-col overflow-hidden bg-[#121017] text-white">
      <header className="grid h-[70px] grid-cols-2 items-center border-b border-white/10 px-5 font-mono text-[10px] tracking-[.13em] text-[#918b99] md:grid-cols-3 md:px-8">
        <button className="flex w-fit items-center gap-2 text-[#bbb5c2]" type="button" onClick={onEnd}><ArrowLeft size={15} /> ENCERRAR</button>
        <span className="hidden justify-self-center md:block">{attempt === 2 ? "↩ SEGUNDA TENTATIVA" : "ENSAIO AO VIVO"}</span>
      ...
      <section className="flex min-h-[430px] flex-1 flex-col items-center justify-center">
        {attempt === 2 && <span className="mb-7 rounded-full border border-[#8f69ef]/40 bg-[#6d45e8]/15 px-4 py-2 text-xs text-[#bea8fb]">Voltamos ao momento escolhido</span>}
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-[#a59eaa]">
          {voiceState === "speaking" ? <><Volume2 size={15} /> {twin.name} está falando...</> : <><i className="size-2 animate-pulse rounded-full bg-[#63d59c]" /> Ouvindo</>}
        </div>
      </section>
      <section className="grid min-h-[220px] gap-5 border-t border-white/10 bg-[#17141c] px-5 py-5 md:grid-cols-[1fr_auto] md:px-[max(28px,calc((100vw-850px)/2))]">
        <div className="flex flex-col justify-end gap-3">
          {lastTurns(transcript, 3).length === 0 ? <p className="text-[#6f6977]">Comece falando. {twin.name} está ouvindo.</p> : lastTurns(transcript, 3).map((turn) => (
            <div className="grid grid-cols-[75px_1fr] gap-2" key={turn.id}><span className={`pt-1 font-mono text-[9px] tracking-[.1em] ${turn.speaker === "customer" ? "text-[#a98bff]" : "text-[#6f6977]"}`}>{turn.speaker === "seller" ? "VOCÊ" : twin.name.toUpperCase()}</span><p className="text-sm leading-6 text-[#d5d0d9]">{turn.text}</p></div>
          ))}
        </div>
        <div className="flex min-w-[230px] flex-col justify-end gap-3">
          {mode === "text" && <form className="flex overflow-hidden rounded-xl bg-[#2a2630]" onSubmit={(event) => { event.preventDefault(); void send(); }}><input className="min-w-0 flex-1 bg-transparent px-3 py-3 text-sm outline-none" aria-label="Sua fala" value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Digite sua fala..." /><button className="px-3 text-[#ad8cff]" type="submit" disabled={sending}><Send size={17} /></button></form>}
          <button className="h-12 rounded-xl border border-white/15 bg-[#2b2731] font-semibold text-[#e9e4ec] hover:bg-[#383240]" type="button" onClick={onEnd}>Encerrar ensaio</button>
        </div>
      </section>
    </main>
  );
...
function ReviewScreen({ analysis, onReplay, onReset }: Readonly<{ analysis: CallAnalysis; onReplay: (id: string) => void; onReset: () => void }>) {
  ...
  if (!moment) return <main className="grid min-h-screen place-items-center"><button className={buttonPrimary} onClick={onReset}>Novo ensaio</button></main>;
  ...
  return (
    <main className="mx-auto min-h-screen w-[min(1040px,calc(100%-32px))] py-10">
      <header className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div><span className="font-mono text-[11px] font-bold tracking-[.14em] text-[#6d45e8]">ANÁLISE DO ENSAIO</span><h1 className="mt-2 text-4xl font-semibold tracking-[-.045em] md:text-5xl">{analysis.moments.length} {analysis.moments.length === 1 ? "momento que vale" : "momentos que valem"} refazer</h1><p className="mt-2 text-[#77717c]">Sem notas inventadas. Só comportamento, contexto e evidência.</p></div>
        <button className={buttonSecondary} type="button" onClick={onReset}>Novo cliente</button>
      </header>
      <article className="rounded-3xl border border-[#e1ded9] bg-white p-5 shadow-[0_20px_60px_rgba(31,24,42,.07)] md:p-9">
        <span className="font-mono text-[10px] font-bold tracking-[.14em] text-[#a36a22]">MOMENTO-CHAVE</span><h2 className="mb-5 mt-2 text-3xl font-semibold">{moment.issue}</h2>
        ...
        <div className="grid gap-3 md:grid-cols-[1.15fr_1fr]">
          <section className="rounded-2xl border border-[#dfeae4] bg-[#f4faf7] p-5"><span className="font-mono text-[10px] font-bold tracking-[.13em] text-[#2f8f65]">CLIENTE</span><blockquote className="my-3 font-serif leading-6">“{moment.customerEvidence[0]?.quote}”</blockquote><small className="flex items-center gap-1 text-[#2f8f65]"><Check size={13} /> Evidência do histórico</small></section>
          <section className="rounded-2xl border border-[#e5dff2] bg-[#f8f6fc] p-5"><span className="font-mono text-[10px] font-bold tracking-[.13em] text-[#6d45e8]">PESQUISA</span>{rules.map((rule) => <div className="mt-3" key={rule.id}><strong className="text-sm">{rule.id === "adaptive-selling" ? "Adaptive selling" : "Follow-up questions"}</strong><p className="mt-1 text-xs text-[#77717c]">{rule.authors.split(" & ")[0]} et al., {rule.year}</p><small className="text-[#928d96]">{rule.journal}</small></div>)}</section>
        </div>
        <div className="mt-4 rounded-2xl bg-[#f2f0ec] p-4"><span className="font-mono text-[9px] font-bold tracking-[.13em] text-[#858088]">OBJETIVO DA SEGUNDA TENTATIVA</span><p className="mt-2 text-sm">{moment.suggestedGoal}</p></div>
        <button className={`${buttonPrimary} mt-5 w-full`} type="button" onClick={() => onReplay(moment.id)}><RotateCcw size={18} /> Refazer deste momento</button>
      </article>
    </main>
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      let message = "Não conseguimos iniciar o áudio.";
      for (const attempt of [0, 1]) {
        try {
          const connection = await connectRealtimeTwin({
            name: state.twin?.name ?? "Cliente",
            instructions,
  ...
  const createTwin = async () => {
    ...
    try {
      ...
      const payload: unknown = await response.json();
      if (!response.ok) throw new Error(typeof payload === "object" && payload && "error" in payload ? String(payload.error) : "Não foi possível criar o espelho.");
      dispatch({ type: "EXTRACTION_SUCCEEDED", twin: payload as CustomerTwin });
    } catch (error) { dispatch({ type: "EXTRACTION_FAILED", message: error instanceof Error ? error.message : "Não foi possível criar o espelho." }); }
  };
  const endPractice = () => {
    connectionRef.current?.close(); connectionRef.current = null;
  ...
  else if (state.phase === "analyzing") screen = <LoadingScreen text="Analisando a conversa..." detail="Cruzando comportamento, contexto e evidência" />;
  ...
  else screen = <main className="grid min-h-screen place-items-center px-5 text-center"><div className="max-w-lg"><span className="mx-auto grid size-12 place-items-center rounded-full bg-[#f2dadd] text-xl font-bold text-[#a43f48]">!</span><h1 className="mt-5 text-3xl font-semibold">Não conseguimos continuar.</h1><p className="mt-3 text-[#77717c]">{state.error}</p><div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">{state.twin && <button className={buttonPrimary} type="button" onClick={() => dispatch({ type: "PRACTICE_STARTED" })}>Tentar novamente</button>}{state.canUseTextFallback && <button className={buttonSecondary} type="button" onClick={() => dispatch({ type: "TEXT_FALLBACK_STARTED" })}>Continuar em texto</button>}<button className={buttonSecondary} type="button" onClick={useDemo}>Usar ensaio de demonstração <ArrowRight size={16} /></button></div><button className="mt-5 text-sm text-[#77717c]" type="button" onClick={() => dispatch({ type: "BACK_TO_TWIN" })}>Voltar</button></div></main>;
